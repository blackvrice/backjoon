...
async function fetchProblems(): Promise<Problem[]> {
    ...
    if (!response.ok || data.ok === false) {
        throw new Error(data.message ?? `문제 목록을 불러오지 못했습니다. (${response.status})`);
    }
...
export default function ProblemsPage() {
    ...
    const recommendedProblemId = filteredProblems[0]?.id ?? problems[0]?.id ?? 1000;
        ...
        >
            <div className="space-y-6">
                <PageHero
                    ...
                    icon={BookOpen}
                    rightTitle="검색 대상 문제"
                    rightValue={isLoading ? "불러오는 중" : problems.length.toLocaleString()}
                ...
                {errorMessage && (
                    <Card className="border-red-100 bg-red-50 p-5">
                        <p className="font-black text-red-700">문제 목록 API 오류</p>
                ...
                <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <StatCard
                        label="전체 문제"
                        value={isLoading ? "-" : problems.length.toLocaleString()}
                ...
                <section>
                    ...
                    {isLoading ? (
                        ...
                        <Card className="border-red-100 bg-red-50 p-5">
                            <p className="font-black text-red-700">문제 목록을 표시할 수 없습니다.</p>
                            <p className="mt-1 text-sm font-bold text-red-500">
                                {errorMessage}
                            </p>
                        </Card>
